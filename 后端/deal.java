...
//        将某用户的历史记录传给前端
            ...
            System.out.println("成功");
            ArrayList<String> showHis = SQL_Operation.searchHis(id);
            //如果某用户历史记录为0，给前端返回一个00字符串
            if (showHis.size() == 0) {
                String num = "0" + Integer.toString(showHis.size());
                System.out.println(num);
                out.write("你没有搜过任何东西哦");
                out.flush();
            }
            //            返回历史记录，|与||格式控制
            else {
                for (int i = 0; i < showHis.size(); i++) {
                    if (i == showHis.size() - 1) {
                        out.write(showHis.get(i));
                        out.flush();
                        System.out.println(showHis.get(i));
                    } else {
                        out.write(showHis.get(i) + ",");
                        out.flush();
                        System.out.println(showHis.get(i));
...
//        将某用户的收藏表传给前端
            ...
            if(showStar.size()==0){
                ...
                System.out.println(num);
                out.write("http://pic25.photophoto.cn/20121115/0017030519176551_b.jpg,你没有收藏任何东西哦,http://yitian.free.svipss.top/ceshi.jsp");
                out.flush();
            }
            //            返回收藏记录，|与||格式控制
            else {
                for (int i = 0; i < showStar.size(); i++) {
                    if (i == showStar.size() - 1) {
                        out.write(showStar.get(i).img+","+showStar.get(i).name+","+showStar.get(i).guanwang+","+showStar.get(i).brower);
                        out.flush();
